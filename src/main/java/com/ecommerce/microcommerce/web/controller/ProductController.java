...
public class ProductController {
    ...
    // ajouter un produit
    // Validation du prix de vente du produit
    ...
    public ResponseEntity<Void> ajouterProduit(@Valid @RequestBody Product product) throws ProduitGratuitException{
        ...
        if(productAdded.getPrix() == 0) 
        	throw new ProduitGratuitException("le prix de vente du produit doit etre superieur à 0");
    ...
    // affichage de la marge entre le prix devente et le prix d'achat
    
    @GetMapping(value="/AdminProduits")
    public Map<Product,Integer> calculerMargeProduit(){
    	Map<Product,Integer> hm = new HashMap<Product, Integer>(); 
    	List<Product> listeProduits = productDao.findAll();
    	int marge ; 
    	for(Product p : listeProduits)
    	{
    		if(p.getPrix() < p.getPrixAchat())
    			marge = 0; 
